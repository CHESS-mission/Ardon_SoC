...
class Briey_LED(config: BrieyConfig) extends Component{
...
//    val coreResetUnbuffered = False
  ...
  val axi = new ClockingArea(axiClockDomain) {
    ...
    val blinking_led = new BlinkingLED
    io.led5 :=  blinking_led.io.led5
...
object BrieyDe0Nano{
  def main(args: Array[String]) {
    ...
    val config = SpinalConfig()
    config.generateVerilog({
      val toplevel = new Briey_LED(BrieyConfig.default.copy(sdramLayout = IS42x160G.layout))
      toplevel
    })
  }
}
